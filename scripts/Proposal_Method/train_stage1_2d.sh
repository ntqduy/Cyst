...
if [[ "${ALLOW_ENV_OVERRIDES:-0}" != "1" ]]; then
  run_direct_config "config/Proposal_Method/stage1_2D/model_experiment_stage1_2d.yaml" "$@"
  exit 0
fi

TAG="stage1_2d_$(tag_or_base "${ENC2D:-}")_$(tag_or_base "${SLICE:-}")_pos$(tag_or_base "${POS:-}")_seed$(tag_or_base "${SEED:-}")"
begin_generated_config "$TAG" "config/Proposal_Method/stage1_2D/model_experiment_stage1_2d.yaml"
