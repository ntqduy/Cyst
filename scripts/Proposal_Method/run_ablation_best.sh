...
# Set OUT_ROOT=outputs/Proposal_Method_Ablation if you want a separate root.
...
if [[ "$RUN_PRETRAIN" == "1" ]]; then
  ...
  for seed in "${SEEDS[@]}"; do
    ...
    run_script_with_env scripts/Proposal_Method/train_stage2_3d.sh "${env_args[@]}"
    for slice in "${SLICES[@]}"; do
      for pos in "${POSITIONS[@]}"; do
        env_args=(SEED="$seed" ENC2D="$best_enc2d" SLICE="$slice" POS="$pos")
        add_env_if_set env_args "${COMMON_PASS_ENV[@]}"
        run_script_with_env scripts/Proposal_Method/train_stage1_2d.sh "${env_args[@]}"
      done
    done
  done
fi
...
for seed in "${SEEDS[@]}"; do
  for slice in "${SLICES[@]}"; do
    for pos in "${POSITIONS[@]}"; do
      ...
      run_script_with_env scripts/Proposal_Method/train_hybrid.sh "${env_args[@]}"
    done
  done
done
