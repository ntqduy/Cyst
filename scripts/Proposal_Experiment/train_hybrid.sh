...
if [[ "${ALLOW_ENV_OVERRIDES:-0}" != "1" ]]; then
  run_direct_config "config/Proposal_Experiment/stage3_hybrid/model_experiment_hybrid.yaml" "$@"
  exit 0
fi
...
begin_generated_config "$TAG" "config/Proposal_Experiment/stage3_hybrid/model_experiment_hybrid.yaml"
