...
if [[ "${ALLOW_ENV_OVERRIDES:-0}" != "1" ]]; then
  run_direct_config "config/Proposal_Experiment/stage2_3D/model_experiment_stage2_3d.yaml" "$@"
  exit 0
fi
...
begin_generated_config "$TAG" "config/Proposal_Experiment/stage2_3D/model_experiment_stage2_3d.yaml"
