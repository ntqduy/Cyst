...
class SlicePositionEncoder(nn.Module):
    ...
    def _normalise_indices_for_feature(self, feature: torch.Tensor, slice_indices: Any | None) -> torch.Tensor:
        if feature.ndim not in {4, 5}:
            raise ValueError(f"Position injection expects a 4D or 5D feature map, got {tuple(feature.shape)}.")
        if slice_indices is None:
            if feature.ndim == 5:
                return torch.zeros((int(feature.shape[0]), int(feature.shape[1])), device=feature.device, dtype=torch.long)
            return torch.zeros(int(feature.shape[0]), device=feature.device, dtype=torch.long)
        if not isinstance(slice_indices, torch.Tensor):
            slice_indices = torch.as_tensor(slice_indices, device=feature.device)
        slice_indices = slice_indices.to(device=feature.device, dtype=torch.long)
        if feature.ndim == 5:
            batch_size, num_slices = int(feature.shape[0]), int(feature.shape[1])
            expected = batch_size * num_slices
            if slice_indices.numel() == 1 and expected > 1:
                slice_indices = slice_indices.expand(expected)
            elif slice_indices.ndim == 1 and slice_indices.numel() == batch_size and num_slices > 1:
                slice_indices = slice_indices[:, None].expand(batch_size, num_slices)
            if slice_indices.numel() != expected:
                raise ValueError(f"slice_indices must contain {expected} values, got {int(slice_indices.numel())}.")
            return slice_indices.reshape(batch_size, num_slices).clamp(0, self.max_positions - 1)
        expected = int(feature.shape[0])
        slice_indices = slice_indices.reshape(-1)
        if slice_indices.numel() == 1 and expected > 1:
            slice_indices = slice_indices.expand(expected)
        if slice_indices.numel() != expected:
            raise ValueError(f"slice_indices must contain {expected} values, got {int(slice_indices.numel())}.")
        return slice_indices.clamp(0, self.max_positions - 1)

    def forward_feature(self, feature: torch.Tensor, slice_indices: Any | None, projection_index: int = 0) -> torch.Tensor:
        projection = self.projections[int(projection_index)]
        embedding = self.embedding(self._normalise_indices_for_feature(feature, slice_indices))
        bias = projection(embedding).to(dtype=feature.dtype, device=feature.device)
        return feature + bias[..., None, None]
