...
def _normalise_decoder_fusion(value):
    ...
    if key in {"same", "same_scale", "unet", "unet_decoder"}:
        return "same_scale"
    if key in {"full", "full_scale", "unet3plus", "unet_3_plus", "all_encoder_plus_previous_decoder"}:
        return "full_scale"
...
class UNetMoE(Module):
    ...
    def __init__(
        ...
        self.model_name = "adaptive_kernel_moe"
        self.backbone_name = "adaptive_kernel_moe_encoder"
    ...
    def _decode_full_scale(self, encoder_features):
        ...
        decoded[2] = self.dropout(decoded[2])
        decoded[1] = self.dropout(decoded[1])
        return {
            "up4": decoded[3],
            "up3": decoded[2],
            "up2": decoded[1],
