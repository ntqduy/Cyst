...
def _normalise_routed_fusion(value):
    key = str(value or "sum").lower().replace("-", "_")
    if key in {"concat", "cat", "concat_1x1", "concat_conv1x1", "concat_conv_1x1"}:
        return "concat_1x1"
    if key in {"sum", "add", "weighted_sum", "weighted_add"}:
        return "sum"
    if key in {"sum_1x1", "add_1x1", "weighted_sum_1x1", "weighted_add_1x1", "sum_conv1x1", "add_conv1x1"}:
        return "sum_1x1"
    raise ValueError("SparseMoE routed_fusion must be concat_1x1, sum, or sum_1x1.")
...
class SparseMoE(Module):
    ...
    def __init__(
        ...
        self.expert_projections = (
            ModuleList([Conv3d(self.out_feat, self.out_feat, kernel_size=1, bias=True) for _ in range(self.num_experts)])
            if self.routed_fusion == "sum_1x1"
            else None
        )
    ...
    def _project_expert_output(self, expert_index, expert_output):
        if self.expert_projections is None:
            return expert_output
        return self.expert_projections[expert_index](expert_output)
    ...
    def _forward_sparse(self, x, gates):
        ...
        for expert_index, expert in enumerate(self.experts):
            ...
            if not bool(selected.any()):
                continue
            expert_output = expert(x[selected])
            expert_output = self._project_expert_output(expert_index, expert_output)
    ...
    def _forward_dense(self, x, gates):
        expert_outputs = torch.stack(
            [self._project_expert_output(expert_index, expert(x)) for expert_index, expert in enumerate(self.experts)],
            dim=1,
        )
...
class UNetMoE(Module):
    ...
    def __init__(
        ...
        moe_routed_fusion="sum",
        moe_dispatch="sparse",
    ):
        # residual: conv for residual input x through 1*1 conv across every layer for downsampling, None for removal of residuals

        super(UNetMoE, self).__init__()
        self.model_name = "adaptive_kernel_moe_add"
        self.backbone_name = "adaptive_kernel_moe_add_encoder"
