...
def _normalise_routed_fusion(value):
    key = str(value or "concat_1x1").lower().replace("-", "_")
    if key in {"concat", "cat", "main_concat", "concat_1x1", "main_concat_1x1", "concat_conv1x1", "concat_conv_1x1"}:
        return "concat_1x1"
    if key in {"sum", "add", "main_sum", "main_add", "weighted_sum", "weighted_add"}:
        return "sum"
    raise ValueError("SparseMoE routed_fusion must be concat_1x1 or sum.")


def _normalise_decoder_fusion(value):
    key = str(value or "same_scale").lower().replace("-", "_")
    if key in {"same", "same_scale", "unet", "standard", "local"}:
        return "same_scale"
    if key in {"full", "full_scale", "unet3plus", "unet_3_plus", "all_encoder", "all_encoder_plus_previous_decoder"}:
        return "full_scale"
    raise ValueError("decoder_fusion must be same_scale or full_scale.")
...
class SparseMoE(Module):
    ...
    DEFAULT_EXPERT_SPECS = (
        {"type": "conv", "kernel": (3, 3, 3), "dilation": (1, 1, 1)},
        {"type": "conv", "kernel": (3, 3, 3), "dilation": (2, 2, 2)},
        {"type": "conv", "kernel": (3, 3, 3), "dilation": (3, 3, 3)},
        {"type": "conv", "kernel": (3, 3, 3), "dilation": (6, 6, 6)},
        {"type": "conv", "kernel": (3, 3, 3), "dilation": (8, 8, 8)},
        {"type": "conv", "kernel": (3, 3, 3), "dilation": (12, 12, 12)},
    )
    ...
    def __init__(
        ...
        self.routed_fusion = _normalise_routed_fusion(routed_fusion)
        ...
        self.expert_fuse = (
            Conv3d(self.out_feat * (self.top_k + 1), self.out_feat, kernel_size=1, bias=True)
    ...
    def _forward_topk_concat(self, x, top_indices, top_gates):
        rank_outputs = [self.main_expert(x)]
        ...
        for rank in range(self.top_k):
            ...
            for expert_index, expert in enumerate(self.experts):
                ...
                weight = rank_gates[selected].to(dtype=expert_output.dtype).view(-1, 1, 1, 1, 1)
                rank_output[selected] = (expert_output * weight).to(dtype=rank_output.dtype)
            rank_outputs.append(rank_output)
        return self.expert_fuse(torch.cat(rank_outputs, dim=1))

    def _forward_sparse(self, x, gates):
        output = self.main_expert(x)
        for expert_index, expert in enumerate(self.experts):
            selected = gates[:, expert_index] > 0
            if not bool(selected.any()):
                continue
            expert_output = expert(x[selected])
            weight = gates[selected, expert_index].to(dtype=expert_output.dtype).view(-1, 1, 1, 1, 1)
            output[selected] = output[selected] + (expert_output * weight).to(dtype=output.dtype)
        return output
    ...
    def _forward_sparse_stream(self, x, gates):
        ...
        for selected, weighted_output, _ in expert_results:
            output[selected] = output[selected] + weighted_output.to(dtype=output.dtype)
        return output

    def _forward_dense(self, x, gates):
        expert_outputs = torch.stack([expert(x) for expert in self.experts], dim=1)
        weights = gates.to(dtype=expert_outputs.dtype).view(gates.shape[0], gates.shape[1], 1, 1, 1, 1)
        return self.main_expert(x) + (expert_outputs * weights).sum(dim=1)
...
class UNetMoE(Module):
    ...
    def __init__(
        ...
        decoder_fusion_channels=None,
    ):
        # residual: conv for residual input x through 1*1 conv across every layer for downsampling, None for removal of residuals

        super(UNetMoE, self).__init__()
        self.model_name = "adaptive_kernel_moe_other"
        self.backbone_name = "adaptive_kernel_moe_other_encoder"
        self.decoder_fusion = _normalise_decoder_fusion(decoder_fusion)
        self.decoder_fusion_channels = (
            int(feat_channels[0]) if decoder_fusion_channels is None else int(decoder_fusion_channels)
        )
        if self.decoder_fusion_channels <= 0:
            raise ValueError("decoder_fusion_channels must be a positive integer.")
        self.architecture_config = {
            ...
            "moe_expert_specs": _serialise_expert_specs(
                tuple(_normalise_expert_spec(item) for item in (moe_expert_kernels or SparseMoE.DEFAULT_EXPERT_SPECS))
            ),
            ...
            "decoder_fusion": self.decoder_fusion,
            "decoder_fusion_channels": int(self.decoder_fusion_channels),
        }
        ...
        self.conv_blk1 = SparseMoE(in_dim, feat_channels[0], expert_kernels=moe_expert_kernels, top_k=moe_top_k, router_noise_std=moe_router_noise_std, noisy_router=moe_noisy_router, router_noise_epsilon=moe_router_noise_epsilon, pre_moe_conv=moe_pre_conv, pre_moe_conv_kernel=moe_pre_conv_kernel, shared_expert=moe_shared_expert, shared_expert_kernel=moe_shared_expert_kernel, shared_expert_weight=moe_shared_expert_weight, routed_fusion=moe_routed_fusion, dispatch=moe_dispatch, residual=residual)
        self.conv_blk2 = SparseMoE(feat_channels[0], feat_channels[1], expert_kernels=moe_expert_kernels, top_k=moe_top_k, router_noise_std=moe_router_noise_std, noisy_router=moe_noisy_router, router_noise_epsilon=moe_router_noise_epsilon, pre_moe_conv=moe_pre_conv, pre_moe_conv_kernel=moe_pre_conv_kernel, shared_expert=moe_shared_expert, shared_expert_kernel=moe_shared_expert_kernel, shared_expert_weight=moe_shared_expert_weight, routed_fusion=moe_routed_fusion, dispatch=moe_dispatch, residual=residual)
        self.conv_blk3 = SparseMoE(feat_channels[1], feat_channels[2], expert_kernels=moe_expert_kernels, top_k=moe_top_k, router_noise_std=moe_router_noise_std, noisy_router=moe_noisy_router, router_noise_epsilon=moe_router_noise_epsilon, pre_moe_conv=moe_pre_conv, pre_moe_conv_kernel=moe_pre_conv_kernel, shared_expert=moe_shared_expert, shared_expert_kernel=moe_shared_expert_kernel, shared_expert_weight=moe_shared_expert_weight, routed_fusion=moe_routed_fusion, dispatch=moe_dispatch, residual=residual)
        self.conv_blk4 = SparseMoE(feat_channels[2], feat_channels[3], expert_kernels=moe_expert_kernels, top_k=moe_top_k, router_noise_std=moe_router_noise_std, noisy_router=moe_noisy_router, router_noise_epsilon=moe_router_noise_epsilon, pre_moe_conv=moe_pre_conv, pre_moe_conv_kernel=moe_pre_conv_kernel, shared_expert=moe_shared_expert, shared_expert_kernel=moe_shared_expert_kernel, shared_expert_weight=moe_shared_expert_weight, routed_fusion=moe_routed_fusion, dispatch=moe_dispatch, residual=residual)
        self.conv_blk5 = SparseMoE(feat_channels[3], feat_channels[4], expert_kernels=moe_expert_kernels, top_k=moe_top_k, router_noise_std=moe_router_noise_std, noisy_router=moe_noisy_router, router_noise_epsilon=moe_router_noise_epsilon, pre_moe_conv=moe_pre_conv, pre_moe_conv_kernel=moe_pre_conv_kernel, shared_expert=moe_shared_expert, shared_expert_kernel=moe_shared_expert_kernel, shared_expert_weight=moe_shared_expert_weight, routed_fusion=moe_routed_fusion, dispatch=moe_dispatch, residual=residual)

        if self.decoder_fusion == "same_scale":
            ...
            self.encoder_projections = torch.nn.ModuleDict()
            self.decoder_projections = torch.nn.ModuleDict()
    ...
    def _decode_same_scale(self, encoder_features):
        x1, x2, x3, x4, base = encoder_features

        d4_up = self._resize_to_reference(self.deconv_blk4(base), x4)
        d4 = torch.cat([d4_up, x4], dim=1)
        d_high4 = self.dec_conv_blk4(d4)

        d3_up = self._resize_to_reference(self.deconv_blk3(d_high4), x3)
        d3 = torch.cat([d3_up, x3], dim=1)
        d_high3 = self.dec_conv_blk3(d3)
        d_high3 = self.dropout(d_high3)

        d2_up = self._resize_to_reference(self.deconv_blk2(d_high3), x2)
        d2 = torch.cat([d2_up, x2], dim=1)
        d_high2 = self.dec_conv_blk2(d2)
        d_high2 = self.dropout(d_high2)

        d1_up = self._resize_to_reference(self.deconv_blk1(d_high2), x1)
    ...
    def _decode_full_scale(self, encoder_features):
        ...
        for target_index in (3, 2, 1, 0):
            target = encoder_features[target_index]
            key = str(target_index)
            projected = []
            for projection, source in zip(self.encoder_projections[key], encoder_features):
                projected.append(self._resize_to_reference(projection(source), target))
            for projection, source_index in zip(self.decoder_projections[key], self._decoder_history_indices(target_index)):
                projected.append(self._resize_to_reference(projection(decoded[source_index]), target))
            decoded[target_index] = self.decoder_fusion_blocks[key](torch.cat(projected, dim=1))
            if target_index in {1, 2}:
                decoded[target_index] = self.dropout(decoded[target_index])
        return {
            "up4": decoded[3],
            "up3": decoded[2],
            "up2": decoded[1],
    ...
    def forward(self, x, return_features: bool = False):
        ...
        encoder_features = [x1, x2, x3, x4, base]
        decoder = self._decode_same_scale(encoder_features) if self.decoder_fusion == "same_scale" else self._decode_full_scale(encoder_features)
        seg = self.one_conv(decoder["final"])
