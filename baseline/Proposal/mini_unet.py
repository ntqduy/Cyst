...
class Conv3Dto2D(Module):
    def __init__(self, hidden_dim, kernel=3, stride=1, padding=1, residual=None, kernel_size=None):
...
class MiniConv3D_Block(Module):
    ...
    def __init__(self, inp_feat, hidden_dim, out_feat=None, kernel=3, stride=1, padding=1, residual=None):
        ...
        self.conv1 = Sequential(
            ...
            BatchNorm3d(hidden_dim),
            ReLU())
        self.miniconv = Conv3Dto2D(hidden_dim, kernel_size=3,
    ...
    def forward(self, x):
        ...
        if not self.residual:
            return self.conv2(self.miniconv(self.conv1(x)))
        else:
            return self.conv2(self.miniconv(self.conv1(x))) + self.residual_upsampler(res)
        
class MiniUnet3D(Module):
    ...
    def __init__(self, in_dim=1, out_dim=2, feat_channels=(64, 256, 256, 512, 1024), residual='conv'):
        ...
        if len(feat_channels) != 5:
            raise ValueError(f"MiniUnet3D expects 4 or 5 feature channels, got {feat_channels}")

        self.model_name = "MiniUnet3D"
        self.backbone_name = "mini_unet3d_encoder"
        ...
        self.conv_blk2 = MiniConv3D_Block(feat_channels[0], feat_channels[1], residual=residual)
        self.conv_blk3 = MiniConv3D_Block(feat_channels[1], feat_channels[2], residual=residual)
        self.conv_blk4 = MiniConv3D_Block(feat_channels[2], feat_channels[3], residual=residual)
        self.conv_blk5 = MiniConv3D_Block(feat_channels[3], feat_channels[4], residual=residual)

        # Decoder convolutions
        self.dec_conv_blk4 = MiniConv3D_Block(2 * feat_channels[3], feat_channels[3], residual=residual)
        self.dec_conv_blk3 = MiniConv3D_Block(2 * feat_channels[2], feat_channels[2], residual=residual)
        self.dec_conv_blk2 = MiniConv3D_Block(2 * feat_channels[1], feat_channels[1], residual=residual)
        self.dec_conv_blk1 = MiniConv3D_Block(2 * feat_channels[0], feat_channels[0], residual=residual)
