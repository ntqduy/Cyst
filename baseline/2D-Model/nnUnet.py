...
def _norm2d(channels: int, normalization: str) -> nn.Module:
...
class ConvBlock2D(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, normalization: str, dropout: float) -> None:
        ...
        layers: list[nn.Module] = [
            ...
            _norm2d(out_channels, normalization),
...
class DownStage2D(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, normalization: str, dropout: float) -> None:
        ...
        self.block = ConvBlock2D(out_channels, out_channels, normalization=normalization, dropout=dropout)
...
class UpStage2D(nn.Module):
    def __init__(self, in_channels: int, skip_channels: int, out_channels: int, normalization: str, dropout: float) -> None:
        ...
        self.block = ConvBlock2D(out_channels + skip_channels, out_channels, normalization=normalization, dropout=dropout)
...
class NNUNet2D(BaseSegmentationModel):
    def __init__(
        self,
        in_channels: int = 1,
        num_classes: int = 2,
        ...
        if len(channels) < 2:
            raise ValueError("NNUNet2D expects at least 2 feature stages.")
        self.model_name = "nnunet"
        self.backbone_name = "nnunet2d_encoder"
        ...
        self.stem = ConvBlock2D(int(in_channels), channels[0], normalization=normalization, dropout=float(dropout))
        self.encoder = nn.ModuleList(
            DownStage2D(channels[index - 1], channels[index], normalization=normalization, dropout=float(dropout))
            for index in range(1, len(channels))
        )
        self.decoder = nn.ModuleList(
            UpStage2D(channels[index], channels[index - 1], channels[index - 1], normalization=normalization, dropout=float(dropout))
