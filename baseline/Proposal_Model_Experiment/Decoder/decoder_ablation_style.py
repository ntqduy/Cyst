...
class FullEncoderSingleDecoder3DDecoder(PartialLoadMixin, nn.Module):
    decoder_name = "full_encoder_single_decoder"
        ...
        **_: Any,
        ...
        for target_index in range(4):
            ...
            self.projections[key] = nn.ModuleList(
                [ConvNormAct3D(source, self.fusion_channels, kernel_size=1, padding=0, normalization=normalization) for source in self.channels]
            )
            self.decoder_projections[key] = nn.ModuleList(
                [ConvNormAct3D(self.channels[source], self.fusion_channels, kernel_size=1, padding=0, normalization=normalization) for source in history]
            )
    ...
    def history_indices(target_index: int) -> list[int]:
        return [] if int(target_index) == 3 else [int(target_index) + 1]
...
class SingleEncoderFullDecoder3DDecoder(PartialLoadMixin, nn.Module):
    decoder_name = "single_encoder_full_decoder"
    ...
    def __init__(
        ...
        **_: Any,
        ...
        for target_index in range(4):
            key = str(target_index)
            history = self.history_indices(target_index)
            self.encoder_projections[key] = ConvNormAct3D(self.channels[target_index], self.fusion_channels, kernel_size=1, padding=0, normalization=normalization)
            self.decoder_projections[key] = nn.ModuleList(
                [ConvNormAct3D(self.channels[source], self.fusion_channels, kernel_size=1, padding=0, normalization=normalization) for source in history]
            )
            self.fusion_blocks[key] = FusionBlock3D(self.fusion_channels * (1 + len(history)), self.channels[target_index], normalization=normalization)
            if self.deep_supervision:
                self.deep_heads[key] = nn.Conv3d(self.channels[target_index], self.num_classes, kernel_size=1)
        self.segmentation_head = nn.Conv3d(self.channels[0], self.num_classes, kernel_size=1)
    ...
    @staticmethod
    def history_indices(target_index: int) -> list[int]:
        return list(range(4, int(target_index), -1))

    def forward(self, skips: list[torch.Tensor] | tuple[torch.Tensor, ...]):
        decoded: dict[int, torch.Tensor] = {4: skips[4]}
        for target_index in (3, 2, 1, 0):
            target_size = tuple(int(item) for item in skips[target_index].shape[-3:])
            key = str(target_index)
            projected = [self.encoder_projections[key](resize3d(skips[target_index], target_size))]
            for projection, source_index in zip(self.decoder_projections[key], self.history_indices(target_index)):
                projected.append(projection(resize3d(decoded[source_index], target_size)))
            decoded[target_index] = self.fusion_blocks[key](torch.cat(projected, dim=1))
        logits = self.segmentation_head(decoded[0])
...
class FullEncoderSingleDecoder2DDecoder(nn.Module):
    def __init__(self, channels: list[int] | tuple[int, ...], num_classes: int = 2, fusion_channels: int | None = None, normalization: str = "batchnorm", **_: Any) -> None:
        ...
        for target_index in range(4):
            key = str(target_index)
            history = FullEncoderSingleDecoder3DDecoder.history_indices(target_index)
            self.projections[key] = nn.ModuleList(
                [ConvNormAct2D(source, self.fusion_channels, kernel_size=1, padding=0, normalization=normalization) for source in self.channels]
            )
            self.decoder_projections[key] = nn.ModuleList(
                [ConvNormAct2D(self.channels[source], self.fusion_channels, kernel_size=1, padding=0, normalization=normalization) for source in history]
            )
    ...
    def forward(self, skips: list[torch.Tensor] | tuple[torch.Tensor, ...]):
        ...
        for target_index in (3, 2, 1, 0):
            ...
            for projection, source_index in zip(self.decoder_projections[key], FullEncoderSingleDecoder3DDecoder.history_indices(target_index)):
                projected.append(projection(resize2d(decoded[source_index], target_size)))
            decoded[target_index] = self.fusion_blocks[key](torch.cat(projected, dim=1))
        return self.segmentation_head(decoded[0])


class SingleEncoderFullDecoder2DDecoder(nn.Module):
    def __init__(self, channels: list[int] | tuple[int, ...], num_classes: int = 2, fusion_channels: int | None = None, normalization: str = "batchnorm", **_: Any) -> None:
        super().__init__()
        self.channels = [int(item) for item in channels]
        self.fusion_channels = int(fusion_channels or self.channels[0])
        ...
        for target_index in range(4):
            key = str(target_index)
            history = SingleEncoderFullDecoder3DDecoder.history_indices(target_index)
            self.encoder_projections[key] = ConvNormAct2D(self.channels[target_index], self.fusion_channels, kernel_size=1, padding=0, normalization=normalization)
            self.decoder_projections[key] = nn.ModuleList(
                [ConvNormAct2D(self.channels[source], self.fusion_channels, kernel_size=1, padding=0, normalization=normalization) for source in history]
            )
            self.fusion_blocks[key] = FusionBlock2D(self.fusion_channels * (1 + len(history)), self.channels[target_index], normalization=normalization)
        self.segmentation_head = nn.Conv2d(self.channels[0], int(num_classes), kernel_size=1)
    ...
    def forward(self, skips: list[torch.Tensor] | tuple[torch.Tensor, ...]):
        ...
        for target_index in (3, 2, 1, 0):
            key = str(target_index)
            target_size = tuple(int(item) for item in skips[target_index].shape[-2:])
            projected = [self.encoder_projections[key](resize2d(skips[target_index], target_size))]
            for projection, source_index in zip(self.decoder_projections[key], SingleEncoderFullDecoder3DDecoder.history_indices(target_index)):
                projected.append(projection(resize2d(decoded[source_index], target_size)))
            decoded[target_index] = self.fusion_blocks[key](torch.cat(projected, dim=1))
        return self.segmentation_head(decoded[0])
...
__all__ = [
    "FullEncoderSingleDecoder2DDecoder",
    "FullEncoderSingleDecoder3DDecoder",
    "SingleEncoderFullDecoder2DDecoder",
    "SingleEncoderFullDecoder3DDecoder",
]
