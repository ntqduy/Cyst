...
class SliceSelector:
    ...
    def __init__(
        ...
        **_: Any,
        ...
        if self.selection_order in {"largest", "high", "higher", "different", "most_different", "diverse", "farthest"}:
            self.select_largest = True
        elif self.selection_order in {"smallest", "low", "lower", "closest", "similar", "most_similar", "mean", "nearest_mean"}:
    ...
    def _proposal_indices_for_one(self, sample: torch.Tensor, depth: int) -> torch.Tensor:
        ...
        for group_index in range(len(boundaries) - 1):
            ...
            local = torch.topk(scores, k=take, largest=self.select_largest).indices
